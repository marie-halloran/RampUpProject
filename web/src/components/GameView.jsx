...
export default function GameView() {
  const { gameId } = useParams();
  const navigate = useNavigate();
  const { game, joinGame, sendMove, setHandlers, leaveGame } = useGameConnection();
  ...
  // Re-join the SignalR group whenever the URL gameId isn't the active game
  // (handles page refresh and direct URL navigation).
  useEffect(() => {
  ...
  async function handleMove(nextBoard, move) {
    ...
    setSyncState('sending');
    try {
      await sendMove(toBoardSnapshot(nextBoard), gameId);
      setSyncState('idle');
    } catch {
      setSyncState('error');
    }
